    ...
    void testRedisPerformance() {
        ...
        try (StatefulRedisConnection<String, String> connection = redisClient.connect()) {
            ...
            for (Integer id : testIds) {
                String json = sync.get("Город:" + id);
                assertNotNull(json, "Данные о городе " + id + " не найдены в Redis");
        ...
        log.info("Redis прочитал 10 городов: " + duration + " ms");
    }
    ...
    void testMySqlPerformance() {
        ...
        try (Session session = sessionFactory.getCurrentSession()) {
            ...
            for (Integer id : testIds) {
                ...
                assertNotNull(city, "Данные о городе " + id + " не найдены в MySQL");
        ...
        long duration = System.currentTimeMillis() - start;
        log.info("MySQL прочитал 10 городов:: " + duration + " ms");
    }
    ...
    private static List<CityCountry> transformData(List<City> cities) {
        return cities.stream().map(city -> {
            ...
            Set<Language> languages = cls.stream().map(cl -> {
                ...
                lang.setLanguage(cl.getId().getLanguage());
